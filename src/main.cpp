...
public:
    ...
    : stop_words_(MakeUniqueNonEmptyStrings(stop_words))
  {
    for (const string& word : stop_words_) {
      if (!IsValidWord(word)) {
        throw invalid_argument("A valid word must not contain special characters");
      }
    }
  }
  ...
  void AddDocument(int document_id, const string& document, DocumentStatus status, const vector<int>& ratings) {
    if ((document_id < 0) || (documents_.count(document_id) > 0)) {
      throw invalid_argument("Document id is less zero or id is repeated");;
    }
...
private:
  ...
  [[nodiscard]] vector<string> SplitIntoWordsNoStop(const string& text) const {
    ...
    for (const string& word : SplitIntoWords(text)) {
      if (!IsValidWord(word)) {
        throw invalid_argument("Word is not valid");
      }
      if (!IsStopWord(word)) {
        words.push_back(word);
      }
  ...
  [[nodiscard]] QueryWord ParseQueryWord(string text, QueryWord& result) const {
    // Empty result by initializing it with default constructed QueryWord
    if (text.empty()) {
      throw invalid_argument("Text of query is empty");
    }
    bool is_minus = false;
    if (text[0] == '-') {
      is_minus = true;
    ...
    if (text.empty() || text[0] == '-' || !IsValidWord(text)) {
      throw invalid_argument("Text of query is not valid");
    }
  ...
  [[nodiscard]] Query ParseQuery(const string& text) const {
    ...
    Query result;
    for (const string& word : SplitIntoWords(text)) {
      QueryWord query_word = ParseQueryWord(word, query_word);
